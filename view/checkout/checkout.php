...
if (isset($_GET)) {
    ...
    $ids = $_GET["ids"];
    $cart_items_id = explode(",", $ids);
    $cart_items = get_cart_by_ids($cart_item_model, $cart_items_id);
    // js calculation
?>
...
<?php
    foreach ($cart_items_id as $cart_item) {
        remove($cart_item_model, $cart_item);
    }
}
?>
<script>
    var cart_number = $("#cart-item span")[0];
    var cart_item = $(".product-cart");
    remain = parseInt(parseInt(cart_number.innerText) - cart_item.length + 1);
    if (remain) {
        cart_number.innerText = remain;
    } else {
        cart_item.hide();
    }
</script>